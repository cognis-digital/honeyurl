"""Native cognis-connect emit for honeyurl — forward findings to any platform.

Maps honeyurl's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    honeyurl ... --format json | honeyurl-emit --to stix
    honeyurl-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out["title"] = f"{rec.get('type', '')} - {rec.get('url', '')}"
        out["severity"] = "medium"
        out["type"] = rec.get("type", "")
        out["description"] = f"Detected {rec.get('type', 'event')} with URL: {rec.get('url', '')}"
        out["tags"] = [rec.get("type", ""), "honeyurl"]
